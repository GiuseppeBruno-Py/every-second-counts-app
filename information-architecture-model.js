...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.CompassoInformationArchitectureModel=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  const reviewAliases=Object.freeze(['weekly','outcomes','analytics']);
  function areaFor(route){if(route==='overview')return'today';if(reviewAliases.includes(route))return'review';return area(route)?.id||view(route)?.area||'today'}
  ...
  function resolve(route,{fallback='today'}={}){if(route==='overview')return'today';if(reviewAliases.includes(route))return'review';return area(route)?.route||view(route)?.route||fallback}
