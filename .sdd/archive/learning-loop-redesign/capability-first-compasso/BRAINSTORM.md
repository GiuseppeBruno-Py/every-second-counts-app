...
**Shipped deliveries:** Slice 1 `Actionable Outcome Foundation` and Delivery 2 `Operational Learning Loop` remain closed and unchanged in their archives.
...
7. Amendment 1 consolidates all remaining roadmap work into two future deliveries: `Capability-first Compasso` and `UX Simplification`.
8. `Capacidades` is the optional cross-cutting learning context, not an isolated screen or a new progress-bearing destination.
9. `IA contextual` is planned for retirement from the product surface only after a dependency/persistence audit; it will not be replaced by an external AI dependency.
10. Notes and Relations are planned for standalone-destination retirement only through `UX Simplification`, while their data and compatibility contracts remain preserved.
...
The previous Slice-1 handoff is historical and has been completed. The next skill is defined by Amendment 1 below.

## 46. Amendment 1 — future roadmap consolidation

**Classification:** architectural roadmap amendment: future-only resequencing plus planned product-surface removal. It changes no shipped requirement, acceptance criterion, implementation, data, or evidence.

### A. Capability-first Compasso

The next major delivery makes a capability the optional learning context across existing surfaces rather than a feature that lives only in `Capacidades`:

`Capability → next attempt → supporting resources → planning/Hoje → session → Evidence → feedback/gap → reflection → next attempt`

It may integrate capability context where it is useful in **Hoje, Estudos, Leituras, sessions, Evidence, Weekly Review, Results, and Consistency**. Association remains optional when it adds no learning value. Studies and Readings remain resources; existing progress semantics remain theirs alone. Capability lifecycle remains active/archived, with no percentage, mastery, completion, confidence, or automatic competence claim.

Its boundary is cross-surface learning context, not a wholesale redesign of every surface. The separate Define must decide only the minimum useful associations, feedback/gap representation, and compatibility behavior needed to make the delivery independently useful.

### B. UX Simplification

This later major delivery removes standalone product destinations for **IA contextual, Notes, and Relations** only after contextual replacements and compatibility audits are defined. It is a product-surface simplification, not data deletion.

`IA contextual` retirement has a mandatory pre-implementation dependency audit covering its current navigation/route registration, `context-rag-feature.js`, `context-learning-feature.js`, manifest/cache composition, commands/listeners, tests, and any persisted settings, generated artifacts, backup/restore participation, or user-visible links. Preserve any persisted data found; do not add or substitute an external AI dependency.

Notes and Relations retirement must preserve useful internal contracts and data: Markdown/vault portability, note data, wikilinks, graph derivation, and JSON backup/restore compatibility. A removed destination must not imply removal of the underlying data, parsing, derivation, export, or recovery path.

### Cascade and compatibility record

- **Untouched and still valid:** both shipped delivery archives, their Define/Design/Build evidence, state `compasso.state.v3`, PWA v72, existing Sessions/Evidence integration, and all shipped acceptance evidence.
- **Superseded only as future planning:** the old multi-slice future map and the historical Slice-1 next-skill handoff in this Brainstorm.
- **No current implementation authorization:** no product code, route, storage, schema, backup, PWA, test, or external-service change is authorized by this amendment.
- **Compatibility constraints for both future deliveries:** preserve existing user data, local-first/offline behavior, JSON backup/restore, Studies/Readings, Markdown/vault portability, note/link/graph contracts, and existing session/Evidence records. Any later schema or surface removal requires a separately approved Define and Design with an explicit migration/rollback path.

### Unresolved decisions for the next Define

1. Which capability associations are valuable enough to expose first in each surface, and which must remain absent?
2. What minimal feedback/gap language and storage is useful without creating a score or a parallel evidence system?
3. How should a retired IA contextual route behave, and which persisted contextual artifacts, if any, require a compatibility view or export path?
4. Which contextual access paths replace standalone Notes and Relations before their destinations are removed?

**Amendment gate: PASS.** The two-delivery roadmap is explicit, capability boundaries and IA-retirement constraints are recorded, and shipped Operational Learning Loop remains unchanged.

## 47. Next skill

Use `$sdd-define` next for **Capability-first Compasso** only. Do not begin `UX Simplification` until the capability-first delivery has its own approved Define/Design boundary.
