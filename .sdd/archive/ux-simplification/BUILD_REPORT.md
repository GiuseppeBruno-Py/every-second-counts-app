...
| 2 | `today-feature.js` | Modify | Complete | AT-01–AT-05, AT-15, AT-19, AT-25 | One derived primary state, safe unavailable states, and continuity commands. |
...
| 19 | `tests/browser/information-architecture-flows.spec.js` | Modify | Complete | AT-01, AT-03–AT-05, AT-19, AT-22, AT-25 | Execute name, stored order, fallback, start, and resume coverage. |
...
| `today-feature.js` | Hoje primary state and commands | Yes |
...
| `tests/browser/information-architecture-flows.spec.js` | Navigation/Execute behavior | Yes |
...
| In-manifest browser matrix | Design closed test manifest | `APP_URL=http://127.0.0.1:4174 npx playwright test tests/browser/capability-context-flows.spec.js tests/browser/information-architecture-flows.spec.js tests/browser/design-system-flows.spec.js tests/browser/critical-flows.spec.js tests/browser/learning-outcome-flows.spec.js --retries=0` | 0 | Pass — 124 pass, 8 project-conditional skip |
...
`APP_URL` points Playwright at the repository's own `scripts/pwa-test-server.js` because the default `python3` web-server command is unavailable on this Windows host. No product or test configuration was changed for that host adaptation.
...
| AT-05 | Pass | Empty Execute fallback opens Today and focuses `Nova ação` without starting a Session. |
...
| AT-22 | Pass | Global Execute has permanent accessible name `Executar`, native keyboard activation, and deterministic focus. |
...
|---|---|---|---|
| None in product implementation | The 20 authorized paths implement the approved behavior. | N/A | No product-scope deviation. |
...
| Risk | Validation used Windows with the repository PWA server rather than remote Linux CI. | Run normal remote CI after a future shipped checkpoint; not a substitute for the current canonical local gate. |
