...
- **Weekly Review:** preserves general and unlinked activity, while `capabilityReflections` records an explicit `keep` or `revise`. Two optional learner-authored fields record a practice worth repeating and whether Evidence changed perceived capability. They never infer a decision, score, trait, signal, or next attempt. `revise` uses the existing outcome update contract and preserves attempt identity/creation time.
