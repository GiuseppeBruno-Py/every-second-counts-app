...
test('history and energy dialogs adopt readable shared notebook type', async ({page}, info) => {
  ...
  for (const dialogs of Object.values(result)) {
    for (const fields of Object.values(dialogs)) {
      for (const [tag, size] of fields) {
        expect(parseFloat(size)).toBeGreaterThanOrEqual(['INPUT','SELECT'].includes(tag) ? 16 : 14);
      }
    }
  }
});
