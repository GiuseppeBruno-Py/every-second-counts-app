...
function renderWeeklyEvidence(evidence) {
  ...
  list.innerHTML = evidence.map(item => {
    ...
    const futureUse=learningOutcomeModel.futureUsePresentation(capabilityRef?.futureUse);
    return `<article class="weekly-evidence-card"><header><b>${escapeHtml(typeLabels[item.type] || 'Evidência')}</b><span>${edited}<time>${escapeHtml(date)}</time></span></header><strong>${escapeHtml(item.summary)}</strong>${item.details ? `<p>${escapeHtml(item.details)}</p>` : ''}<footer>${escapeHtml(linked?.title || (resolved.outcome?.capability) || 'Item removido')} · ${escapeHtml(domainLabels[item.domain] || item.domain)}</footer>${capabilityRef?`<p><b>${resolved.available?`Capacidade: ${escapeHtml(resolved.outcome.capability)}`:'Capacidade indisponível'}</b><br>Tentativa: ${escapeHtml(resolved.attemptText)}${futureUse?`<br><span class="future-use-context">Uso na execução: ${escapeHtml(futureUse.label)}</span>`:''}</p>`:''}<div class="weekly-evidence-actions">${resolved.available?`<button type="button" data-weekly-open-capability="${escapeHtml(capabilityRef.outcomeId)}">Abrir capacidade</button>`:''}<button type="button" data-evidence-edit="${escapeHtml(item.id)}">Editar</button><button type="button" class="danger" data-evidence-delete="${escapeHtml(item.id)}">Excluir</button></div></article>`;
  }).join('');
}
...
function weeklyUpdateDecisionCard(card,{focusAttempt=false}={}){const decision=card?.querySelector('[data-weekly-decision]'),wrap=card?.querySelector('[data-weekly-attempt-wrap]'),attempt=card?.querySelector('[data-weekly-attempt]'),futureUse=card?.querySelector('[data-weekly-future-use]');if(!decision||!wrap)return;const revise=decision.value==='revise';wrap.hidden=!revise;if(!revise){if(attempt)attempt.value=card.dataset.attemptText||attempt.value;if(futureUse)futureUse.value=card.dataset.futureUse||''}if(revise&&focusAttempt)requestAnimationFrame(()=>attempt?.focus())}
...
async function saveWeeklyReview() {
  ...
  for(const card of document.querySelectorAll('[data-weekly-capability]')){
    ...
    const futureUse=card.querySelector('[data-weekly-future-use]');if(decision.value==='revise'&&futureUse?.value&&!learningOutcomeModel.normalizeFutureUse(futureUse.value)){weeklySetDecisionError('Escolha uma forma válida de usar esta capacidade.',futureUse);return false}
  }
  ...
  for(const card of document.querySelectorAll('[data-weekly-capability]')){
    ...
    const reflection=card.querySelector('[data-weekly-reflection]').value.trim(),decidedAttemptText=card.querySelector('[data-weekly-attempt]').value.trim(),selectedFutureUse=card.querySelector('[data-weekly-future-use]')?.value||'';
    ...
    const futureUseChanged=decision==='revise'&&selectedFutureUse!==(outcome.nextAttempt.futureUse||'');
    if(current&&current.decision===decision&&current.reflection===reflection&&current.decidedAttemptText===(decision==='keep'?capabilityRef.attemptText:decidedAttemptText)&&!futureUseChanged)continue;
    if(decision==='revise')outcomes=outcomes.map(item=>item.id===outcomeId?learningOutcomeModel.updateOutcome(item,{nextAttempt:{text:decidedAttemptText,futureUse:selectedFutureUse||null}},{now}):item);
